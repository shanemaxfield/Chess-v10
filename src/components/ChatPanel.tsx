...
export default function ChatPanel() {
  ...
  const executePlan = (plan: any, fallbackResponse: string): { responseText: string, followUps?: string[] } => {
    ...
    if (plan.arrows && plan.arrows.length > 0) {
      ...
      setArrows(newArrows)
      if (responseText === fallbackResponse && !plan.moves) {
    ...
    if (plan.highlights && plan.highlights.length > 0) {
      ...
      if (responseText === fallbackResponse && !plan.moves && !plan.arrows) {
        responseText = `Highlighted ${plan.highlights.reduce((acc: number, h: any) => acc + h.squares.length, 0)} square(s)`
      }
    }
